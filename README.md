...
- 🔄 **Dynamic API Routing**: Automatic ngrok URL synchronization via Google Apps Script
...
└──────────────┘     └────────┬─────────┘     │ │  │  + QLoRA Adapter         │  │ │
                              ...
                              │               │ │                                │ │
                    ┌─────────▼─────────┐     │ │  POST /generate                │ │
                    │  Google Apps      │     │ │  GET  /status                  │ │
                    │  Script Webhook   │◀────│ └────────────────────────────────┘ │
                    │                   │     │                │                   │
                    │  Stores & serves  │     │       ┌────────▼────────┐          │
                    │  current ngrok URL│     │       │   ngrok tunnel  │          │
                    └───────────────────┘     │       │  (public URL)   │          │
                                              │       └─────────────────┘          │
                                              └────────────────────────────────────┘
```
...
3. **API URL Resolution**: Next.js API route fetches current ngrok URL from Apps Script
4. **Model Inference**: Request forwarded to Colab-hosted FastAPI → Falcon3 generates post
...
| **Google Apps Script** | Dynamic ngrok URL management |
...
sequenceDiagram
    ...
    participant C as Colab + FastAPI
    participant M as Falcon3 Model
    
    U->>F: Enter topic + settings
    F->>A: Fetch current ngrok URL
    A-->>F: Return ngrok URL
    F->>C: POST /generate {topic, tone, length}
    C->>M: Generate with prompt template
    M-->>C: Generated text
    C->>C: Post-process (regex cleanup)
    C-->>F: {generated_text, processing_time}
    F-->>U: Display with typing animation
```
...
3. **URL Auto-Sync**: ngrok URL pushed to Google Apps Script webhook
4. **Frontend Ready**: Next.js app fetches URL dynamically from Apps Script
5. **Generate Posts**: Users interact with chat interface, requests routed through tunnel
...
!pip install transformers==4.51.2 accelerate==1.6.0 peft==0.15.1 \
    bitsandbytes==0.45.5 trl==0.16.1 torch==2.6.0 sentencepiece==0.2.0 \
    fastapi==0.115.12 uvicorn==0.34.2 pyngrok==7.2.9 huggingface-hub==0.32.3 \
    protobuf==6.31.1 psutil==7.0.0
```
...
```python
session_active = start_session()
```

This will:
- Start FastAPI server on port 8000
- Create ngrok tunnel
- Auto-update frontend with new ngrok URL
...
│   │
│   └── lib/
│       ├── api-config.ts      # Dynamic ngrok URL fetching
│       ├── supabase/
│       │   ├── client.ts
│       │   └── server.ts
│       ├── three-setup.ts
